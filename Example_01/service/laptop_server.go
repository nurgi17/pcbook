...
func (server *LaptopServer) CreateLaptop(ctx context.Context, req *pb.CreateLaptopRequest) (*pb.CreateLaptopResponse, error) {
	...
	if err != nil {
		...
		return nil, status.Errorf(code, "cannot save laptop to the serve: %v", err)
	}
...
func (server*LaptopServer) UploadImage(stream pb.LaptopService_UploadImageServer) error {
	...
	log.Printf("receive an upoad-image for laptop %s with image type %s", laptopID, imageType)
...
func contextError(ctx context.Context) error {
	...
	case context.Canceled:
		return logError(status.Error(codes.Canceled, "request cancelled by client"))
